		...
		<%@include file="../common/header.jsp" %>
			...
			<form action="${pageContext.request.contextPath}/UserEditServlet" method="post"
				...
				<div class="input-wrap">
					...
					<input type="text" name="userAddress" value="${user.address}" id="userAddress" class="form_input"
						required>
				</div>
				<div
					class="input-wrap input-wrap_check ${'1'.equals(adminFlag) ? '' : 'hidden' || meetingRoom.user.isAdmin == '0' ? 'hidden' : ''}">
			...
			<form action="${pageContext.request.contextPath}/AdminUserServlet" method="post"
				class="form form_unsubscribe ${cancelFlag == '1' ? '' : 'hidden'}">
				<input type="hidden" name="userId" value="${user.id}">
				...
				<input type="hidden" name="cancelFlag" value="${cancelFlag}">
				<input type="hidden" name="adminFlag" value="${adminFlag}">
				<input type="submit" name="action" value="退会する" class="button_submit button_unsubscribe"
					onclick="return confirm('本当に退会してよろしいですか？');">
			</form>
