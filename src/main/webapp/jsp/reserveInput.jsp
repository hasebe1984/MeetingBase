				...
				<%@ page import="jp.co.sys.bean.ReservationBean" %>
					<%@ page import="jp.co.sys.util.RoomList" %>
						<%@ page import="jp.co.sys.dao.UserDao" %>
							...
							<% //=====サーブレットから受け取る=====MeetingRoom meetingRoom=(MeetingRoom)
								session.getAttribute("meetingRoom"); session.setAttribute("meetingRoom", meetingRoom);
								String[] period=meetingRoom.getPeriod(); RoomList rooms=meetingRoom.getRooms();
								...
								<%@include file="../common/header.jsp" %>
									...
									<h2>予約可能時間帯 &nbsp;&nbsp;&nbsp;
										<c:out value="${meetingRoom.user.name}" /> さん
									</h2>
									...
									<%--後で復活タグ<h1>予約可能時間帯${meetingRoom.user.name}</h1>--%>
										...
										<table class="input_table">
											...
											<%--二重for文　i=会議室名の表示--%>
												<% for (int i=0; i < rooms.size(); i++) { String
													room=rooms.get(i).getName(); request.setAttribute("room", room); %>
													<tbody>
														<tr>
															<td>
																<c:out value="${room }" />
